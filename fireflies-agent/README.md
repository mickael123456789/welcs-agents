...
**Доступ команды по кодовому слову.** Владелец (`TELEGRAM_CHAT_ID`) — всегда внутри.
Остальные при первом обращении должны прислать боту кодовое слово; после этого их
чат попадает в разрешённые и им открыты все режимы. Кодовое слово задаётся переменной
`BOT_ACCESS_CODE` (по умолчанию `WELCS2026`). Список авторизованных хранится локально
(`.bot_authorized.json`) и сбрасывается при перезапуске процесса — тогда участники
вводят слово ещё раз.
