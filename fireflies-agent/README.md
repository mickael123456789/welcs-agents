...
Бот **приватный**. Владелец (`TELEGRAM_CHAT_ID`) — все режимы. Сотрудникам можно
выдать доступ к отдельным режимам через `team_access.json` (лежит в репозитории,
переживает перезапуски):

```json
{ "users": { "123456789": { "name": "Аня", "modes": ["bitrix"] } } }
```

`modes` — любой набор из `fireflies, bitrix, priorities, board, morning, notion`.
Человеку показываются только его кнопки. Свой ID легко узнать командой `/myid`.
Все, кого нет в списке (и не владелец), получают вежливый отказ.
