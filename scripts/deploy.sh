...
# 4. остановить локальные демоны на Mac — теперь бот живёт в облаке,
#    иначе два слушателя на один токен дадут конфликт Telegram (409).
launchctl bootout "gui/$(id -u)/com.welcs.fireflies-bot" 2>/dev/null && echo "⏹ локальный бот остановлен" || true
launchctl bootout "gui/$(id -u)/com.welcs.fireflies-agent" 2>/dev/null && echo "⏹ локальные сводки остановлены" || true

echo
echo "✅ ГОТОВО. Бот и сводки теперь работают в облаке GitHub — компьютер можно выключать."
echo "   Репозиторий: https://github.com/$FULL"
echo "   Логи и запуски: https://github.com/$FULL/actions"
echo "   Проверь Telegram: нажми кнопку (🔥/📊/🎯/🧠) и задай вопрос."
